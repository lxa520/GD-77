...
internal class CodeplugComms
{
	...
	public void startCodeplugReadOrWriteInNewThread()
	{
		...
		switch (CommunicationMode)
		{
			case CommunicationType.codeplugRead:
                // just reuse the readData hook for now ...
                this.thread = new Thread(this.hackyReadFlash);
    ...
    public void hackyReadFlash()
    {
        int block = 0;
        byte[] flashdata = new byte[160]; // buffer for individual transfers
        FileStream outputFile = File.Create("full_flash_1.dat");
        while (true)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    specifiedDevice.ReceiveData(flashdata);
                    if (flashdata[0] != CodeplugComms.CMD_ACK[0])
                    {
                        if (this.OnFirmwareUpdateProgress != null)
                        {
                    ...
                    else
                    {
                        for (int idx = 0; idx < 1; idx++)
                        {
                            ...
                            adr[1] = (byte)block;
                            adr[2] = 1;
                            specifiedDevice.SendData(adr);

                            Array.Clear(flashdata, 0, flashdata.Length);
                            specifiedDevice.ReceiveData(flashdata);
                            outputFile.Write(flashdata, 0, 16);
                            outputFile.Flush();
                            Console.WriteLine("Reading block: "+adr[0] + "/" + adr[1] + "/" + adr[2]);
                        }
                    }
                }
            }
            ...
            // workaround: wait 2 seconds before next block starts, otherwise the hacked firmware will collapse ...
